...
class Slack_manager:
    ...
    count_file = "Android/android_DB/android_started_count.json"  # 카운트를 저장할 파일 경로
    ...
    def test_started_message(cls):
        cls.load_count()
        cls.started_count += 1
        message = f"🤖 [TravelWallet_WebPage] Android_자동화 테스트 [시작]합니다!\n\n 실행횟수 : {cls.started_count}"
    ...
    def send_update_message(cls, success, additional_message=None):
        ...
        message = (f"🤖 [TravelWallet_WebPage] Android_자동화 테스트 [업데이트]\n\n"
                   f"{pass_message}\n{fail_message}")
    ...
    def final_summary_message(cls):
        ...
        message = (f"📝 [TravelWallet_WebPage] Android_자동화 테스트 [종료] 최종 결과:\n\n"
