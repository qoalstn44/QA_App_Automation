import time
import json
from android_refund.android_test_refund import My_Wallet_Minimum_Reset
...
from Functions import country_data, click_element, find_elements, click_element_with_retry, handle_step, minimum_charge_click, minimum_charge_all, check_element_exists
import sys
import os
...
def perform_Minimum_reset(driver, country_xpath, country_code_to_name):
    country_name = get_country_name(country_xpath, country_code_to_name)
    My_Wallet_Minimum_Reset.Minimum_reset(driver, country_name)
...
class My_Wallet_Minimum_Charge:
    @classmethod
    def Minimum_charge_and_refund_all(cls, driver):
                ...
                while not success:  # 특정 국가 충전 재시도 루프
                    try:
                        def execute_charge_steps():
                            ...
                            time.sleep(3)
                                                # 국가 코드(예: USD, EUR 등) 추출
                            currency = country_xpath.split('_')[2]  # 예시: USD, JPY, EUR 등
                            minimum_charge_data = minimum_charge_all()

                            # 최소 충전 금액을 입력하는 함수 호출
                            minimum_charge_click(driver, currency, minimum_charge_data)
                        ...
                        perform_Minimum_reset(driver, country_xpath,
                                              country_code_to_name)                        
                        Slack_manager.test_completed_message(f"{country_xpath} [최소] 충전, 환불 완료")
                        success = True
