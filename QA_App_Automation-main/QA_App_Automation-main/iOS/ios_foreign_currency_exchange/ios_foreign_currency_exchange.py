...
from ios_refund.ios_test_refund import KRW_to_foreign_exchange, random_to_foreign_exchange
from selenium.common.exceptions import StaleElementReferenceException
from ios_common.ios_slack import Slack_manager
from ios_refund.ios_test_refund import exchange_status_checker
from ios_common.ios_pass_word import pass_word
from Functions import country_data, click_element, find_elements, click_element_with_retry, handle_step, click_element_with_scroll
            ...
            for country_xpath in countries:
                ...
                if country_xpath.endswith(f"_{cls.country_code}_xpath"):
                    def KRW_execute_charge_steps():
                        ...
                        pass_word.pass_word_info(driver)
                        time.sleep(2)
                        completion_status = handle_step(lambda: find_elements(
...
class Randem_Foreign_Currency_Exchange:
    ...
    def random_foreign_currency_to_foreign_currency_exchange(driver):
        ...
        def randem_execute_charge_steps():
            ...
            extract_and_calculate_and_click(driver, elements)
            handle_step(lambda: click_element_with_retry(driver, onboarding_data["next_xpath"], "다음 선택"), f"{C002} - Failed at next_button_xpath")
            time.sleep(0.5)
            ...
            completion_status = handle_step(lambda: find_elements(driver, charge_completed_data["charge_completed"], "충전 완료 텍스트 확인"), "Failed at charge_completed")
            ...
            if not completion_status:
                ...
                handle_step(lambda: click_element_with_retry(driver, charge_completed_data["charge_close"], "닫기 선택"), f"{C005} - Failed at charge_close")
                random_perform_reset(driver, country_xpath, country_code_to_name)
